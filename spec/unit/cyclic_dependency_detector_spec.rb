...
RSpec.describe Dry::System::CyclicDependencyDetector do
  describe ".detect_from_backtrace" do
    it "delegates to instance method" do
      backtrace = ["/path/to/foo.rb:10:in `initialize'"]
      detector_instance = instance_double(described_class)

      allow(described_class).to receive(:new).with(backtrace).and_return(detector_instance)
      allow(detector_instance).to receive(:detect_cycle).and_return(%w[foo])

      result = described_class.detect_from_backtrace(backtrace)
      expect(result).to eq(%w[foo])
    end
  end
  ...
  describe "#detect_cycle" do
    context "with no component files in backtrace" do
      ...
      it "returns empty array" do
        detector = described_class.new(backtrace)
    ...
    context "with more than 4 unique components" do
      ...
      it "returns first 4 components as fallback" do
        detector = described_class.new(backtrace)
        result = detector.detect_cycle
        expect(result).to eq(%w[service_a service_b service_c service_d])
      end
    end
  end
  ...
  describe "#find_component_cycle" do
    ...
    context "with insufficient component files" do
      it "returns empty array for less than 4 files" do
        result = detector.send(:find_component_cycle, %w[foo bar])
        expect(result).to eq([])
      end
    end
  ...
  describe "integration scenarios" do
    context "real-world backtrace simulation" do
      ...
      it "correctly identifies the cyclic dependencies" do
        detector = described_class.new(realistic_backtrace)
        result = detector.detect_cycle
        expect(result).to eq(%w[user_service auth_service])
      end
    end
    ...
    context "complex cycle with multiple components" do
      ...
      it "detects the three-component cycle" do
        detector = described_class.new(complex_backtrace)
        result = detector.detect_cycle
        expect(result).to eq(%w[service_a service_b service_c])
      end
    end
  end
end
